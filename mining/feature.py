...
def get_feature_wo_energy(df, FFTSAMPLE):
    header_list = ['proximity', 'ambient', 'leanForward']
    ...
    COV_M = np.cov(R_T.T)
    COV = np.array([COV_M[0, 1], COV_M[0, 2], COV_M[1, 2]])

    H_T = hstack((M_T, V_T, MAX, MIN, MED, SK_T, RMS_T, K_T, Q1, Q3, QI,
                  prox_fft, amb_fft, lean_fft,
                  count_above_mean, count_below_mean,
                  first_location_of_maximum, first_location_of_minimum,
                  longest_strike_above_mean, longest_strike_below_mean,
                  number_cwt_peaks,
                  SK_prox_fft, K_prox_fft, SK_amb_fft, K_amb_fft, SK_lean_fft, K_lean_fft,
                  COV))
