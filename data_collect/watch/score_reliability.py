...
def Gobi_Zain():
    filePath = '../../Data/203/inwild_unzip/data_acc/acc.csv'
    # filePath = '0103/data_acc/acc.csv'
    timestampCol = 1
    saveFolder = 'second'
    unit = 'second'
    sensorFreq = 20
    ...
    print(len(df))
    df = pd.to_numeric(df['Time'], errors='ignore')
    print(df.head)
    timeArr = df.values
    ...
    # requirement: unixtimestamp must be in milliseconds
    countDf = calc_reliability(timeArr, sensorFreq, unit, plot=0)
    # countDf.to_csv(os.path.join(saveFolder, 'Zain_reliability.csv'), index=False)
